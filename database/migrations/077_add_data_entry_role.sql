-- Migration 077: Add Data Entry Role
-- Adds 'data_entry' role for users who can manage transaction data but cannot access reports/analytics

-- Step 1: Add 'data_entry' to the user_role enum
ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'data_entry';

-- Step 2: Update the user_has_permission function to include data_entry in hierarchy
...
-- Step 3: Update main_transaction policies to allow data_entry CRUD (except delete)
...
-- Step 4: Update original_transaction policies similarly
...
-- Step 5: Update import_batch policies (if table exists)
DO $$
BEGIN
...
-- Step 6: Create helper function to check if user is data_entry or lower
...
-- Migration complete!
-- Summary:
-- 1. Added 'data_entry' role to user_role enum
-- 2. Updated user_has_permission function with new role hierarchy (5 levels instead of 4)
-- 3. Updated main_transaction policies to allow data_entry CRUD (except delete - that's still admin+)
-- 4. Updated original_transaction policies to allow data_entry CRUD (except delete)
-- 5. Updated import_batch policies to allow data_entry to import
-- 6. Added helper function to check if user is data_entry or lower (for UI restrictions)
--
-- Role Hierarchy:
-- Owner (5) - Full access
-- Admin (4) - Manage accounts, users (except owner actions)
-- Editor (3) - Full transaction access
