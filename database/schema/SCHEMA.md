...
### 17. `scheduled_payments`
Tracks contractual payment obligations and recurring payments (Migration 059)

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `scheduled_payment_id` | SERIAL | PRIMARY KEY | Auto-incrementing ID |
| `entity_id` | UUID | NOT NULL, FOREIGN KEY → entities(id) ON DELETE CASCADE | Entity this schedule belongs to |
| `category_id` | INTEGER | NOT NULL, FOREIGN KEY → categories(category_id) ON DELETE RESTRICT | Payment category |
| `contract_id` | INTEGER | NULLABLE, FOREIGN KEY → contracts(contract_id) ON DELETE SET NULL | Linked contract (if any) |
| `payment_type` | TEXT | NULLABLE | Free-form schedule name (e.g., "Year 1", "Rent", "Utilities") |
| `contract_name` | VARCHAR(255) | NOT NULL | Contract/payment name |
| `contract_type` | VARCHAR(50) | NOT NULL | 'lease', 'service', 'construction', 'subscription', 'other' |
...
| `schedule_type` | VARCHAR(20) | NOT NULL | 'recurring', 'one_time', 'custom_dates' |
| `frequency` | VARCHAR(20) | NULLABLE | 'monthly', 'quarterly', 'yearly', 'custom' |
| `payment_day` | INTEGER | NULLABLE, CHECK 1-31 | Day of month for recurring payments |
| `start_date` | DATE | NOT NULL | Schedule start date |
| `end_date` | DATE | NULLABLE | Schedule end date (ongoing if NULL) |
...
**Updated in**: Migration 061 (removed payment_type constraint), Migration 063 (view update)

---

### 18. `scheduled_payment_instances`
Individual payment due dates generated from scheduled payments (Migration 059)

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `instance_id` | SERIAL | PRIMARY KEY | Auto-incrementing ID |
| `scheduled_payment_id` | INTEGER | NOT NULL, FOREIGN KEY → scheduled_payments(scheduled_payment_id) ON DELETE CASCADE | Parent schedule |
| `due_date` | DATE | NOT NULL | Payment due date |
| `amount` | DECIMAL(15,2) | NOT NULL, CHECK > 0 | Instance amount |
| `status` | VARCHAR(20) | DEFAULT 'pending' | 'pending', 'paid', 'overdue', 'cancelled' |
| `paid_date` | DATE | NULLABLE | When payment was made |
| `paid_amount` | DECIMAL(15,2) | NULLABLE, CHECK > 0 | Amount actually paid |
| `transaction_id` | INTEGER | NULLABLE, FOREIGN KEY → main_transaction(main_transaction_id) ON DELETE SET NULL | Linked transaction |
| `notes` | TEXT | NULLABLE | Instance notes |
| `created_at` | TIMESTAMPTZ | DEFAULT NOW() | Creation timestamp |
| `updated_at` | TIMESTAMPTZ | DEFAULT NOW() | Last update timestamp |
...
### 19. `contracts`
Master contract agreements for payment schedules (Migration 060)

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `contract_id` | SERIAL | PRIMARY KEY | Auto-incrementing ID |
| `entity_id` | UUID | NOT NULL, FOREIGN KEY → entities(id) ON DELETE CASCADE | Entity this contract belongs to |
| `contract_number` | VARCHAR(100) | NOT NULL | Unique contract reference |
| `contract_name` | VARCHAR(255) | NOT NULL | Contract title |
| `contract_type` | VARCHAR(50) | NOT NULL | 'lease', 'service', 'construction', 'subscription', 'other' |
| `counterparty` | VARCHAR(255) | NOT NULL | Other party to the contract |
| `effective_date` | DATE | NOT NULL | Contract start date |
| `expiration_date` | DATE | NULLABLE | Contract end date (ongoing if NULL) |
| `auto_renew` | BOOLEAN | DEFAULT FALSE | Auto-renewal flag |
| `notice_period_days` | INTEGER | NULLABLE | Required notice for termination |
| `total_value` | DECIMAL(15,2) | NULLABLE | Total contract value |
| `status` | VARCHAR(20) | DEFAULT 'active' | 'active', 'expired', 'terminated' |
| `notes` | TEXT | NULLABLE | Contract notes |
| `created_at` | TIMESTAMPTZ | DEFAULT NOW() | Creation timestamp |
| `updated_at` | TIMESTAMPTZ | DEFAULT NOW() | Last update timestamp |
...
### 20. `contract_amendments`
Tracks changes and addenda to contracts (Migration 060)

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `amendment_id` | SERIAL | PRIMARY KEY | Auto-incrementing ID |
| `contract_id` | INTEGER | NOT NULL, FOREIGN KEY → contracts(contract_id) ON DELETE CASCADE | Parent contract |
| `amendment_number` | VARCHAR(50) | NOT NULL | Amendment reference |
| `amendment_date` | DATE | NOT NULL | When amendment was signed |
| `effective_date` | DATE | NOT NULL | When amendment takes effect |
...
### 21. `category_budgets`
Monthly/periodic budgets for expense categories (Migration 053)
...
| `category_id` | INTEGER | NOT NULL, FOREIGN KEY → categories(category_id) ON DELETE CASCADE | Budget category |
| `budget_amount` | DECIMAL(15,2) | NOT NULL, CHECK > 0 | Budget amount for period |
...
### 3. `scheduled_payment_overview`
Comprehensive view of scheduled payments with aggregated instance data (Migration 059, Updated 063)

**Purpose**: Provides scheduled payment information with instance counts, payment totals, and next due date

**Columns Include**:
- All `scheduled_payments` fields (including `contract_id` added in Migration 063)
- `category_name`: From categories table
- Instance summaries:
  - `total_instances`: Total payment instances generated
  - `pending_count`: Number of pending instances
  - `paid_count`: Number of paid instances
  - `overdue_count`: Number of overdue instances
- Payment totals:
  - `total_paid`: Sum of paid amounts
  - `total_pending`: Sum of pending amounts
  - `total_overdue`: Sum of overdue amounts
- `next_due_date`: Earliest pending payment due date

**Definition** (from Migration 063):
```sql
SELECT
  sp.*, c.category_name,
  COUNT(spi.*) as total_instances,
  COUNT(*) FILTER (WHERE spi.status = 'pending') as pending_count,
  COUNT(*) FILTER (WHERE spi.status = 'paid') as paid_count,
  COUNT(*) FILTER (WHERE spi.status = 'overdue') as overdue_count,
...
GROUP BY sp.scheduled_payment_id, c.category_name;
```

**Created in**: Migration 059
**Updated in**: Migration 063 (added contract_id column)
